...
int
getListFile(char *DirPath, char *ListFile)
{
    ...
    if (dir)
	{
	    while ((dirnode = readdir(dir)) != NULL)
		{
			...
			{
			    if ((strlen(ListFile) + strlen(dirnode->d_name))
				    > LIST_FILE_MAX)
				{
...
int
getFileFromFtp(char *FtpServerIP, char *FileName, char *UserName,
        char *Password)
{
...
initDefaultConfigFile()
{
    ...
    char device_name[128] = "MBox.MAC"; //assign MAC address
    FILE *config_file;
...
int
changeRoomName(char *old_name, char *new_name)
{
